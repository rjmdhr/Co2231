...
public class Graph<T> implements GraphADT<T>
{
    ...
    public void removeVertex(int index)
    {
        ...
        {
            // remove vertex at index and modify vertices array
            for (int i = index; i < numVertices; i++)
                vertices[i] = vertices[i+1];

            // modify adjaceny matrix to accomodate vertex removal
            // shift vertices from rows below vertex to be removed
            for (int row = index; row < numVertices; row++)
                for (int col = 0; col <= numVertices; col++)
                    adjMatrix[row][col] = adjMatrix[row+1][col];

            //shift vertices from colomns right of vertex to be removed
            for (int col = index; col < numVertices; col++)
    ...
    public boolean isConnected()
    {
        ...
        for (Iterator<T> it = iteratorBFS(0); it.hasNext();)
        {
            count++;
            it.next();
        }
    ...
    {
        Object[] ob = new Object[numVertices];
